...
import {render, renderProjectTodoCard, renderDueSoonTodocard} from './js/render';
...
sectionTodo.addEventListener('click', function(e) {
    if (e.target.classList.contains('btn__label')) {rotateTodo(e)}
    if (e.target.className === 'btn todo-card__back--btn') {rotateTodo(e)}
});
sectionTodo.addEventListener('click', function(e) {
    if (e.target.className === 'btn btn__delete btn--small') {removeTodo(e)}
})
sectionTodo.addEventListener('click', function(e) {
    if (e.target.className === 'btn btn__complete btn--small') {completeTodo(e)}
})

sectionFooter.addEventListener('click', function(e) {
    if (e.target.className === 'project-container__card' || e.target.className === 'heading-quinary project-container__card--heading') {renderProjectTodoCard(e.target.textContent)}
  
})

sectionSidebar.addEventListener('click', function(e) {
    if (e.target.className === 'sidebar-card') {renderDueSoonTodocard(e)}
    if (e.target.className === 'sidebar-card') {console.log(e)}

})

todoForm.addEventListener('click', function(e) {

    if (e.target.id === 'form-submit') {validateForm(e)};
})
